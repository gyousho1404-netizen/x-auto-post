...
def generate_tweet(post_type):
    ...
    if post_type == "blog":
        prompt = base + "テーマ：ブログ更新のお知らせ。「本日のブログ更新しました！」で始め、補助金・行政書士に関する内容を1文添える。ハッシュタグは#補助金と#行政書士の2つ。絵文字1〜2個。"
    elif post_type == "jizokuka":
        prompt = base + "テーマ：小規模事業者持続化補助金第20回が公募中。対象は小規模事業者。相談を促す内容。最適なハッシュタグ2〜3個。絵文字1〜2個。"
    elif post_type == "osaka":
        prompt = base + "テーマ：大阪府「利益率向上賃上げ事業」の案内。大阪府の制度であることを明記。#大阪 #賃上げ #補助金などのハッシュタグ2〜3個。絵文字1〜2個。"
    else:
